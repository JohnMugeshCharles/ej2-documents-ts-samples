...
const sbArray: string[] = ['angular', 'nextjs', 'react', 'javascript', 'aspnetcore', 'aspnetmvc', 'vue', 'blazor'];
/**
...
let accessibilityButton: Button = new Button({ content: 'Check Now!', cssClass: "e-link" });
accessibilityButton.appendTo('#sf-axe-btn');

let msgTemplate: Message = new Message({
        severity: "Info",
        cssClass: "sf-axe-msg"
});
msgTemplate.appendTo('#sf-axe-message');

// Hide the accessibility message for AI samples and on mobile viewports
function toggleAxeMessageVisibility(): void {
    var wrapper = document.querySelector('.sf-axe-section');
    if (!wrapper) { return; }

    var hashStr: string = (location.hash || '').replace(/^#\//, '');
    var hashControl: string = hashStr.split('/')[2] || '';
    var aiControlRegex: RegExp = /\b(?:ai-(?!assistview\b)[a-z-]+|[a-z-]*smart[a-z-]*|[a-z-]*summariz[a-z-]*)\b/i;
    var isAi: boolean = aiControlRegex.test(hashControl);
    var isPdf: boolean = hashStr.indexOf('/pdf/') !== -1;

    wrapper.classList.toggle('sb-hide', isMobile || isAi || isPdf);
}
...
function postProcessApiTable(): void {
    const gridHost = document.getElementById('api-grid');
    if (!gridHost) { return; }
    const blocks = Array.from(gridHost.querySelectorAll('.sb-sample-description')) as HTMLElement[];
    for (const block of blocks) {
        let inner = block.querySelector('.sb-api-content') as HTMLElement;
        if (!inner) {
            const wrap = createElement('div', { className: 'sb-api-content' }) as HTMLElement;
        ...
        if (block.querySelector('#showtag') || block.querySelector('#hidetag')) { continue; }
        const showTag = createElement('a', { id: 'showtag', innerHTML: ' show more...' }) as HTMLElement;
        const hideTag = createElement('a', { id: 'hidetag', innerHTML: ' hide less..' }) as HTMLElement;
        hideTag.classList.add('e-display');
        showTag.addEventListener('click', () => tagShowmore(block));
        hideTag.addEventListener('click', () => taghideless(block));
...
function renderApiTable(data: any[]): void {
    if (!apiGridHost) { apiGridHost = document.getElementById('api-grid'); }
    if (!apiGridHost) { return; }
    apiGridHost.innerHTML = '';
    const grid = createElement('div', { className: 'e-grid e-gridhover e-responsive e-default' });
    const header = createElement('div', { className: 'e-gridheader' });
    const headerContent = createElement('div', { className: 'e-headercontent' });
    const hTable = createElement('table', { className: 'e-table', attrs: { role: 'grid' } });
    const hColGroup = createElement('colgroup');
    hColGroup.appendChild(createElement('col', { attrs: { style: 'width: 180px' } }));
    hColGroup.appendChild(createElement('col', { attrs: { style: 'width: 180px' } }));
    hColGroup.appendChild(createElement('col', { attrs: { style: 'width: 200px' } }));
    const thead = createElement('thead', { className: 'e-columnheader' });
    const htr = createElement('tr', { className: 'e-columnheader' });
    const th1 = createElement('th', { className: 'e-headercell', attrs: { style: 'text-align: center;' } }); th1.textContent = 'Name';
    const th2 = createElement('th', { className: 'e-headercell' }); th2.textContent = 'Type';
    const th3 = createElement('th', { className: 'e-headercell' }); th3.textContent = 'Description';
    ...
    const content = createElement('div', { className: 'e-gridcontent' });
    const contentInner = createElement('div', { className: 'e-content' });
    const cTable = createElement('table', { className: 'e-table', attrs: { role: 'grid' } });
    const cColGroup = hColGroup.cloneNode(true);
    const tbody = createElement('tbody');
    const rows: any[] = data || [];
    for (let i = 0; i < rows.length; i++) {
        const r = rows[i];
        const tr = createElement('tr', { className: 'e-row' });
        const td1 = createElement('td', { className: 'e-rowcell e-centeralign', attrs: { 'data-col': 'name' } });
        const a = createElement('a', { attrs: { href: r.link || '#', target: '_blank', rel: 'noopener noreferrer' } });
        a.textContent = r.name || '';
        td1.appendChild(a);
        const td2 = createElement('td', { className: 'e-rowcell', attrs: { 'data-col': 'type' } });
        td2.textContent = r.type || '';
        const td3 = createElement('td', { className: 'e-rowcell', attrs: { 'data-col': 'description' } });
        const descWrap = createElement('div', { className: 'sb-sample-description' });
        const descInner = createElement('div', { className: 'sb-api-content' });
    ...
    tbody.addEventListener('mouseover', (e: any) => {
        const tr = (e.target as HTMLElement).closest('tr');
        if (tr) { tr.classList.add('e-hover'); }
    });
    tbody.addEventListener('mouseout', (e: any) => {
        const tr = (e.target as HTMLElement).closest('tr');
...
function bindEvents(): void {
    ...
    select("#sf-axe-btn").addEventListener('click', () => {
        runAxeReport();
    });
}
...
function setSbLink(): void {
    const hrefLink: string[] = location.hash.split('/').slice(2);
    const component: string = hrefLink[0];
    ...
    let ej2PlatformHash: string = `#/${selectedTheme}/${hrefLink.join('/')}`;
    if (ej2PlatformHash.endsWith('.html')) {
        ej2PlatformHash = ej2PlatformHash.replace('.html', '');
    }
    const sdkPlatforms: { [key: string]: string } = {
    ...
    for (let i: number = 0, len: number = sbArray.length; i < len; i++) {
        let sb: string = sbArray[i];
        let ele: HTMLFormElement = <HTMLFormElement>select('#' + sb);
        let sampleUrl: string = `${baseUrl}${sdkPlatforms[sb]}/`;
        ...
        if (component === 'pdf' && ['aspnetcore', 'angular', 'react', 'vue', 'javascript'].indexOf(sb) === -1) {
            if (ele) {
                (ele.parentElement as HTMLElement).style.display = 'none';
            }
            continue;
        }
        if (sb === 'aspnetcore') {
            ...
            let nextjsPlatformHash: string = `${selectedTheme}/${hrefLink.join('/')}`;
...
function renderLeftPaneComponents(): void {
    ...
    let initialComponent = location.hash.split('/')[2] || '{{:component}}';
    ...
    // Hide the control tree and show the samples list
    let controlTree = select('#controlTree') as HTMLElement;
    let controlSamples = select('#controlSamples') as HTMLElement;
    
    if (controlTree && controlSamples) {
        controlTree.style.display = 'none';
        controlSamples.style.display = '';
        // Update the control name in the back button
        let controlName = select('.control-name');
        if (controlName) {
    ...
    for (let node of samplesList) {
        ...
        for (let subNode of samples) {
            ...
            crossRoads.addRoute(urlString, () => {
                ...
                const baseUrl = 'https://www.syncfusion.com/';
                let canonicalUrl: string = baseUrl;
                if (control === 'pdf') {
                    canonicalUrl += 'document-sdk/javascript-pdf-library';
                }
                else if (control === 'pdfviewer') {
                    canonicalUrl += 'pdf-viewer-sdk/javascript-pdf-viewer';
                }
                else if (control === 'document-editor') {
                    canonicalUrl += 'docx-editor-sdk/javascript-docx-editor';
                }
                else if (control === 'spreedsheet') {
                    canonicalUrl += 'spreadsheet-editor-sdk/javascript-spreadsheet-editor';
                }

...
(window as any).default = (): void => {
  ...
  function pageClick(args: { x: number, y: number }): void {
    if (isDropped) {
      ...
      // Validate and adjust bounds: ensure field stays within page dimensions
      let pageIndex: number = viewer.currentPageNumber - 1;
      let pageWidth: number = viewer.viewerBase.pageSize[pageIndex].width;
      let pageHeight: number = viewer.viewerBase.pageSize[pageIndex].height;

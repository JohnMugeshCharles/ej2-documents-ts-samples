...
(window as any).default = (): void => {
    ...
    class CustomAdaptor extends ODataAdaptor {
        public processResponse(): Object {
            ...
            let original: { result: Record<string, any>[], count: number } = super.processResponse.apply(this, arguments);
            original.result.forEach((item: Record<string, any>[], idx: number) => {
                result[idx] = {};
                Object.keys(item).forEach((key: string) => {
                    if (['OrderID', 'CustomerID', 'ShipName', 'ShipCity', 'ShipCountry'].indexOf(key) > -1) {
                        (result[idx] as Record<string, any>)[key] = item[key as any];
                    }
                });
            });
